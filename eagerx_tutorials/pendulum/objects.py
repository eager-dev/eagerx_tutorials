...
class Pendulum(Object):
    ...
    @register.engine_states(model_state=Float32MultiArray, model_parameters=Float32MultiArray, mass=Float32, length=Float32, max_speed=Float32)
    ...
    def agnostic(spec: ObjectSpec, rate: float):
        ...
        spec.sensors.dtheta.space_converter = SpaceConverter.make("Space_Float32", low=-8, high=8, dtype="float32")
        ...
        mass_low = 0.02
        mass_high = 0.025
        spec.states.mass.space_converter = SpaceConverter.make(
            "Space_Float32", low=mass_low, high=mass_high, dtype="float32"
        )
    ...
    def ode_bridge(spec: ObjectSpec, graph: EngineGraph):
        ...
        spec.OdeBridge.states.model_parameters = EngineState.make("OdeParameters", list(range(8)))
        spec.OdeBridge.states.mass = EngineState.make("DummyState")
        spec.OdeBridge.states.length = EngineState.make("DummyState")
        spec.OdeBridge.states.max_speed = EngineState.make("DummyState")
